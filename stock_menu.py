...
def add_stock(stock_list):
    ...
    while option != "0":
        ...
        option=input("Stock Added - Press <<Enter>> to Add Another Stock or 0 to Stop:")
...
def list_stocks(stock_list):
    print("--- Stock List ---")
    print("SYMBOL"," " * (14-len("SYMBOL")), "NAME"," " * (14-len("NAME")), "SHARES"," " * (14-len("SHARES")))
    for stock in stock_list:
...
def add_stock_data(stock_list):
   ...
   for stock in stock_list:
       print(stock.symbol, " ", end="")
   print("]")
   symbol = input(print("Which stock do you want to use?: ")).upper()
   found = False
   for stock in stock_list:
       if stock.symbol == symbol:
           found = True
        ...
        while True:
            ...
            for stock in stock_list:
                if stock.symbol == symbol:
                    ...
                    current_stock = stock
            if found == True:
                # print('type(current_stock.shares)',type(current_stock.shares))
...
def display_chart(stock_list):
    ...
    for stock in stock_list:
        print(stock.symbol+" ",end="")
    print("]")
    symbol=input("Select the symbol to disply the chart for: ").upper()
    found=False
    for stock in stock_list:
        if stock.symbol==symbol:
            found=True
    ...
    while True:
        ...
        if(choice == "S" or choice == "s"):
            ...
            try:
              ...
              print("Stock data saved to file \"stock_data.json\" successfully")
            except Exception as e:
                print("Error: Stock data file not created!!")
                print(e)
                break
            finally:
                f.close()
        if choice == "L" or choice == "l":
            try:
                ...
                print("Read stock file data successfully")
            except Exception as e:
                print("Error: Could not read file successfully!!!")
                print(e)
            finally:
                f.close()
        if choice == "D" or choice == "d":
            print("Historical Data will be added")
